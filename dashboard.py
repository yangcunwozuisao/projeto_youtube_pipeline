...
Correção: MONGO_URI e MONGO_DB eram hard-coded; agora lidos via
variáveis de ambiente, permitindo deployment em servidores remotos/Docker.
"""
...
def load_data() -> tuple[pd.DataFrame, str]:
    ...
    if brands_count > 0:
        records = list(db["brands"].find({}, {"_id": 0}))
        df      = pd.DataFrame(records)
        source  = f"MongoDB › brands ({brands_count} docs)"
    else:
        ...
        if videos.empty:
            raise ValueError("Coleção 'videos' vazia. Execute o pipeline primeiro.")

        df = videos
        for sub in [nlp, topics]:
            if not sub.empty:
                extra_cols = [c for c in sub.columns if c not in df.columns or c == "videoId"]
                df = df.merge(sub[extra_cols], on="videoId", how="left")

        source = "MongoDB › videos + nlp + topics (brands vazia)"

    num_cols = [
        "viewCount", "likeCount", "commentCount", "dislikeCount",
        "subscriberCount", "channelVideoCount", "channelViewCount",
        "sent_value", "topc_likes", "topc_conf", "topc_value", "brand_conf",
    ]
    for col in num_cols:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors="coerce")

    str_cols = ["brand_primary", "sent_label", "topc_label", "channelTitle", "model_hint"]
    for col in str_cols:
        if col in df.columns:
...
def load_spam_data() -> pd.DataFrame | None:
    ...
    if count == 0:
        return None
    records = list(db["comments"].find({"spam_label": {"$exists": True}}, {"_id": 0}))
    df = pd.DataFrame(records)
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if "sent_label" in df.columns:
            ...
            selected_sent = st.multiselect("Sentimento do vídeo", sent_labels, default=sent_labels)

        selected_topc = None
        if "topc_label" in df.columns:
            comment_labels = sorted(df["topc_label"].dropna().astype(str).unique().tolist())
            selected_topc  = st.multiselect("Sentimento do comentário", comment_labels, default=comment_labels)
    ...
    with col_a:
        if {"viewCount", "likeCount"}.issubset(df_f.columns):
            hover = [c for c in ["title", "channelTitle", "brand_primary"] if c in df_f.columns]
            color = "brand_primary" if "brand_primary" in df_f.columns else None
            fig   = px.scatter(
                df_f, x="viewCount", y="likeCount",
                color=color, hover_data=hover,
                log_x=True, log_y=True,
                labels={"viewCount": "Views (log)", "likeCount": "Likes (log)"},
    ...
    with col_b:
        if has_subscribers and "viewCount" in df_f.columns:
            hover = [c for c in ["channelTitle", "brand_primary"] if c in df_f.columns]
            color = "brand_primary" if "brand_primary" in df_f.columns else None
            fig   = px.scatter(
                df_f, x="subscriberCount", y="viewCount",
                color=color, hover_data=hover,
                log_x=True, log_y=True,
                labels={"subscriberCount": "Inscritos (log)", "viewCount": "Views (log)"},
    ...
    with col_c:
        if {"viewCount", "commentCount"}.issubset(df_f.columns):
            hover = [c for c in ["title", "channelTitle"] if c in df_f.columns]
            color = "brand_primary" if "brand_primary" in df_f.columns else None
            fig   = px.scatter(
                df_f, x="viewCount", y="commentCount",
                color=color, hover_data=hover,
                log_x=True, log_y=True,
                labels={"viewCount": "Views (log)", "commentCount": "Comentários (log)"},
    ...
    with col_d:
        if has_subscribers and "likeCount" in df_f.columns:
            ...
            color = "brand_primary" if "brand_primary" in df_f.columns else None
            fig   = px.scatter(
                df_f, x="subscriberCount", y="likeCount",
                color=color, hover_data=hover,
                log_x=True, log_y=True,
                labels={"subscriberCount": "Inscritos (log)", "likeCount": "Likes (log)"},
    ...
    if "channelTitle" in df_f.columns:
        ...
        if has_subscribers:
            agg_dict["subscriberCount"] = ("subscriberCount", "first")
        ...
        ch_display = ch_agg.rename(columns={k: v for k, v in rename_map.items() if k in ch_agg.columns})

        for col in ["Views (total)", "Likes (total)", "Comentários (total)", "Inscritos"]:
            if col in ch_display.columns:
                ch_display[col] = ch_display[col].apply(lambda x: fmt_big(x) if pd.notna(x) else "—")
        ...
        top15 = ch_agg.head(15)
        fig_ch = px.bar(
            top15, x="channelTitle", y="viewCount",
            color="likeCount" if "likeCount" in top15.columns else None,
            color_continuous_scale="Blues",
            labels={"channelTitle": "Canal", "viewCount": "Views", "likeCount": "Likes"},
            title="Top 15 canais por views totais (cor = likes totais)",
        )
        fig_ch.update_layout(height=380, xaxis_tickangle=-35)
    ...
    if {"like_rate", "sent_value"}.issubset(df_f.columns):
        hover = [c for c in ["title", "channelTitle", "brand_primary", "viewCount"] if c in df_f.columns]
        color = "brand_primary" if "brand_primary" in df_f.columns else None
        fig = px.scatter(
            df_f, x="sent_value", y="like_rate",
            color=color, hover_data=hover,
            labels={"sent_value": "Sentimento do vídeo", "like_rate": "Like rate (%)"},
    ...
    with col1:
        if "brand_primary" in df_f.columns:
            ...
            st.plotly_chart(
                px.bar(bc, x="brand_primary", y="count", title="Distribuição de marcas"),
                use_container_width=True,
            )

    with col2:
        if "topc_label" in df_f.columns and has_comments:
            tc = df_f["topc_label"].fillna("unknown").value_counts(dropna=False).reset_index()
            tc.columns = ["topc_label", "count"]
            st.plotly_chart(
                px.pie(tc, names="topc_label", values="count",
    ...
    with col3:
        if {"brand_primary", "sent_value"}.issubset(df_f.columns):
            bs = (
                ...
                .mean().reset_index().sort_values("sent_value", ascending=False)
            )
            st.plotly_chart(
                px.bar(bs, x="brand_primary", y="sent_value",
    ...
    with col4:
        if {"brand_primary", "topc_value"}.issubset(df_f.columns) and has_comments:
            bc2 = (
                ...
                .mean().reset_index().sort_values("topc_value", ascending=False)
            )
            st.plotly_chart(
                px.bar(bc2, x="brand_primary", y="topc_value",
    ...
    cols_show = [c for c in [
        ...
        "viewCount", "likeCount", "commentCount", "like_rate", "comment_rate",
        "subscriberCount",
        "brand_primary", "model_hint",
        "sent_label", "sent_value",
        "top_comment", "topc_label", "topc_value",
    ] if c in df_f.columns]
    ...
    if spam_df is None:
        ...
        with sm2:
            st.metric("Spam / bots", f"{n_spam}  ({n_spam/total_com*100:.1f}%)", delta_color="off")
        with sm3:
            st.metric("Suspeitos", f"{n_suspeito}  ({n_suspeito/total_com*100:.1f}%)")
        with sm4:
            st.metric("Legítimos", f"{n_legitimo}  ({n_legitimo/total_com*100:.1f}%)")

        st.divider()

        sc1, sc2 = st.columns(2)
        color_map = {"spam": "#E24B4A", "suspeito": "#EF9F27", "legítimo": "#1D9E75"}
        ...
        with sc2:
            fig_hist = px.histogram(
                spam_df, x="spam_score",
                color="spam_label", color_discrete_map=color_map,
                nbins=12, title="Distribuição de spam score",
                labels={"spam_score": "Score", "count": "Comentários"},
            )
            fig_hist.add_vline(x=3, line_dash="dash", line_color="#EF9F27",
                               annotation_text="suspeito ≥ 3")
            fig_hist.add_vline(x=5, line_dash="dash", line_color="#E24B4A",
        ...
        for sig_str in spam_df["spam_signals"].dropna():
            all_signals.extend([s for s in sig_str.split("|") if s])
        ...
        if "videoId" in spam_df.columns:
            ...
            st.dataframe(spam_by_video.head(20), use_container_width=True, hide_index=True)

        st.subheader("Comentários marcados como spam")
        spam_only = spam_df[spam_df["spam_label"] == "spam"].sort_values(
            "spam_score", ascending=False
        )
        cols_spam = [c for c in ["videoId", "author", "text", "likeCount",
                                  "spam_score", "spam_signals"] if c in spam_only.columns]
        st.dataframe(spam_only[cols_spam].head(50), use_container_width=True, hide_index=True)

        with st.expander("Ver comentários suspeitos"):
            suspeitos = spam_df[spam_df["spam_label"] == "suspeito"].sort_values(
                "spam_score", ascending=False
            )
            st.dataframe(suspeitos[cols_spam].head(100), use_container_width=True, hide_index=True)
